...
public class PendienteCajaViewBeanDAO implements DAO<Object, PendienteCajaView> {

	@PersistenceContext
	private EntityManager em;

	public PendienteCajaView create(PendienteCajaView t) {
		this.em.persist(t);
		return t;
	}

	public void delete(PendienteCajaView t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public PendienteCajaView find(Object id) {
		return this.em.find(PendienteCajaView.class, id);
	}

	public PendienteCajaView update(PendienteCajaView t) {
		return this.em.merge(t);
	}

	public List<PendienteCajaView> findAll() {
		List<PendienteCajaView> list = null;
		CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
		cq.select(cq.from(PendienteCajaView.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}
	
	public List<PendienteCajaView> findRange(int[] range) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<PendienteCajaView> rt = cq.from(PendienteCajaView.class);
	...
	public List<PendienteCajaView> findByNamedQuery(String namedQueryName) {
	...
	public List<PendienteCajaView> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<PendienteCajaView> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
